...
| [Confirmation email selection](sprint/current/done/BUG-010-stale-confirmation-email-returned-after-resend.md) (+ [BUG-012](sprint/current/done/BUG-012-footer-postcode-extracted-as-otp.md), [US-080](sprint/current/done/US-080-llm-email-code-extraction.md), [BUG-013](bugs/BUG-013-unconsumed-stale-email-after-resend.md) open) | `agent/email_codes.py` (`wait_for_confirmation`, `_search_folder`, `extract_code`) + `email_extract.py` (validation) + `run_agent.py` (`get_email_code`) | Handing back a message already used — or an unconsumed sibling a Resend just invalidated — feeds the site a dead code, and the retry loop cannot end. The twins are a wrong-length token entered as though it were the code, and a codeless email answering its footer's postcode. US-080's LLM reader adds the invented token: extraction may hallucinate, so validation must demand the literal token in the email — verbatim, and of the stated width | `agent/tests/test_email_codes.py`, `agent/tests/test_email_extract.py` |
