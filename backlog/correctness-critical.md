...
| [TOTP shared secret](sprint/current/US-059-otp-and-social-login-in-tested-flows.md) | `agent/totp_codes.py` (`TotpSecret`) | The shared secret is permanent, not a 10-minute code — any leak into a step, event, log, or report is a standing second factor, not a stale one; also: a wrong code from a step-boundary or padding bug silently fails the login rather than erroring loud | `agent/tests/test_totp_codes.py` |

Every row above was written assertion-first — the guard reviewed before the
implementation — except these five, which are covered test-*alongside* and
escalate on their next change: **Scheduler claim**, **Slot math**,
**Redaction**, **Magic-link consume**, **Tenant isolation**.
