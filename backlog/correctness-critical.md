...
| [Confirmation email selection](sprint/current/done/BUG-010-stale-confirmation-email-returned-after-resend.md) | `agent/email_codes.py` (`wait_for_confirmation`, `_search_folder`) + `run_agent.py` (`get_email_code`) | Handing back a message already used feeds the site the code its Resend invalidated, and the retry loop cannot end — the agent reports the *site* as rejecting fresh codes. The twin is a wrong-length token entered as though it were the code | `agent/tests/test_email_codes.py` |
