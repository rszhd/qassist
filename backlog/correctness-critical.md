...
| [Saved browser sessions](sprint/current/done/US-043-reusable-authenticated-sessions.md) | `server/src/browserSession.js` + `runs.js` (spawn, `done` capture, `close`) + `agent/browser_session.py` + `015_browser_sessions.sql` | A `storageState` **is** the credential, and `scrub` can never be the guard because the blob never enters the LLM's context. Handed over as a dict it silently loads nothing; a teardown that unlinks only the file it wrote leaves the credential in browser-use's `.bak` forever | `session-blob.test.js`, `session-containment.test.js`, `agent/tests/test_browser_session.py`, `session-postgres.test.js` (real Postgres) |
