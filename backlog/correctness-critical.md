...
| [Step-boundary ordering](unscheduled/US-074-run-agent-pure-logic-extracted.md) | `agent/step_events.py` (`callback`) | Advance the attribution before flushing and a step's findings are filed against the next step, or spend the per-step cap on a chatty first step and the step that *fails* reports nothing — a clean evidence section on a failed run | **Assertion pending** — the seam is `callback`'s injected `report_blocks`/`flush_diagnostics`/`set_step`; `agent/tests/test_step_events.py` covers the assembly only |
...
**Staging/production config separation** has no guard at all yet, and
**Step-boundary ordering** is waiting on its reviewed assertion.
