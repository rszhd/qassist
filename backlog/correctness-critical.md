...
| [Per-user concurrency cap](sprint/current/done/US-028-per-user-concurrency-limit.md) + [override](sprint/current/done/US-058-per-user-concurrency-override.md) | `server/src/concurrency.js` + `runs.js` (`createRun`, `canStart`, `startNext`) + `routes/helpers.js` + `012_user_concurrency_override.sql` | A gate that counts running-only, or an override reaching some of the three gates and not all, lets one account park runs in the queue and beat the FIFO; a stale cache makes the operator's lever a no-op until a restart they cannot afford | `concurrency-cap.test.js`, `concurrency-fairshare.test.js`, `concurrency-off.test.js`, `concurrency-override.test.js`, `concurrency-override-off.test.js`, `concurrency-override-postgres.test.js` (real Postgres), `concurrency-override-route.test.js` |
| [Staging/production config separation](sprint/current/done/US-038-staging-environment.md) | `docker-compose.prod.yml` (`env_file`, `ports`, the `${COMPOSE_PROJECT_NAME}` labels) + `.env.staging` | A second stack on the same box inherits a production value and stops being staging — live Stripe keys, production's `SESSION_SECRET`, real mail recipients, a republished port or a duplicated router name | **Candidate — no assertion yet.** Needs Docker in CI, so it waits on US-032; the assertion is to be written and reviewed first |
| [BYOK-only run funding](sprint/current/done/US-039-byok-only-no-server-key.md) | `server/src/openaiKey.js` (`resolveRunKey`) + `routes/helpers.js` + `runs.js` (`startRun`'s child env) + `scheduler.js` | A start path resolves no key and starts anyway — or `...process.env` in the spawn hands the server's ambient key to the child, restoring the tier at the one layer that spends money. Inverse: the solo self-hoster is stranded with nowhere to put a key | `byok-only.test.js`, `byok-solo.test.js`, `byok-postgres.test.js` (real Postgres), `boot.test.js` |
...
| [Activation window](sprint/current/done/US-054-activation-window-after-subscribe.md) | `server/src/activation.js` + `routes/helpers.js` (`requireEntitled`) + the scheduler's fire path + `010_account_activation.sql` | The window leaks into the unset path and a self-host grows a wall it never asked for; a start path misses it; an ordinary webhook rewrites `activated_at` and re-walls a month-old customer; or the deadline slides forward forever | `activation-gate.test.js`, `billing-gate.test.js` (the window OFF) |
| [Stopping a run](sprint/current/done/US-047-stop-a-run.md) | `server/src/runs.js` (`stopRun`, `verdictOf`, `close`) + `routes/runs.js` + `011_run_cancelled_status.sql` + `frontend/src/RunView.jsx` | The loudest failure looks like success: browser-use returns history normally out of `Agent.stop()`, so an aborted run ends `passed` — green in CI, in History and in the mail. Quieter twins: the slot released twice or never, a terminal status assigned at *request* time, a WS event the view paints green | `stop-run.test.js`, `notify.test.js`, `RunView.test.jsx` |
| [Navigation confinement](sprint/current/done/US-042-agent-navigation-confinement.md) | `server/src/navigationPolicy.js` + `runs.js` + `agent/navigation_policy.py` + `013_navigation_confinement.sql` | A fence that is off by default, or that lets one spelling of an address through, is worth *less* than no fence, because it is believed. Also: a project allowlist that silently re-opens the instance floor, and an empty allowlist browser-use reads as "allow everything" | `navigation-policy.test.js`, `navigation-fence.test.js`, `navigation-fence-off.test.js`, `navigation-fence-postgres.test.js` (real Postgres), `agent/tests/test_navigation_policy.py` |
| [Project fixtures](sprint/current/done/US-048-file-upload-in-test-flows.md) | `server/src/fixtures.js` + `runs.js` (`QA_FIXTURES`) + `routes/fixtures.js` + `agent/fixtures.py` + `014_project_fixtures.sql` | The `available_file_paths` whitelist is the only thing between an agent argued into `read_file` and the container's `.env`. A traversal spelling, another tenant's project, or an ad-hoc run resolving to "everything on the box" all get on it | `fixture-path.test.js`, `fixture-whitelist.test.js`, `agent/tests/test_fixtures.py` |
| [Captured browser evidence](sprint/current/done/US-044-network-and-console-evidence.md) | `agent/diagnostics.py` + `run_agent.py`'s diagnostics path + `runs.js` (`diagnosticsOf`) | Page-authored text reaches the PDF unscrubbed — or truncated *before* scrubbing, which splits the secret so `scrub` no longer matches it. And a per-run budget wearing a per-step budget's clothes records nothing for the step that failed | `agent/tests/test_diagnostics.py`, `diagnostics-evidence.test.js`, `agent/tests/test_report_diagnostics.py` |
| [Saved browser sessions](sprint/current/done/US-043-reusable-authenticated-sessions.md) | `server/src/browserSession.js` + `runs.js` (spawn, `done` capture, `close`) + `agent/browser_session.py` + `015_browser_sessions.sql`; extended by US-063 with `sessionCapture.js`, `routes/capture.js`, `018_extension_session_capture.sql`, `extension/lib/storageState.js` | A `storageState` **is** the credential, and `scrub` can never be the guard because the blob never enters the LLM's context. Handed over as a dict it silently loads nothing; a teardown that unlinks only the file it wrote leaves the credential in browser-use's `.bak` forever. US-063 adds a second writer and a fifth end (the abandoned capture) through a door US-043 never had: a browser extension holding the blob outside the server between reading it and posting it, gated by a single-use capture token rather than a full-privilege API key | `session-blob.test.js`, `session-containment.test.js`, `agent/tests/test_browser_session.py`, `session-postgres.test.js` (real Postgres), `session-capture-token.test.js`, `extension/lib/storageState.test.mjs` |
| [Stored secret variables](sprint/current/done/US-064-secret-variables-in-a-scheduled-run.md) | `server/src/testSecrets.js` + `variables.js` (`normalizeDeclarations`, `secretWrites`, `resolveForRun`) + `routes/tests.js` + `017_test_secrets.sql` | A masked read plus a naive write-back erases the credential during an unrelated edit, silently, and it surfaces as a failed run at 02:00 a fortnight later. Twins: a blank override read as an override, and an empty secret typed into a password field as if it were one | `test-secrets.test.js`, `test-secrets-postgres.test.js` (real Postgres), `variables.test.js`, `scheduler.test.js` |
...
| [Run memory metric](sprint/current/done/US-024-memory-watchdog-pss-metric.md) | `server/src/procMemory.js` + `runs.js` (`memWatch`) + `config.js` (`MAX_RUN_MEMORY_MB`) | The result meets one threshold and nothing else, so a unit slip — kB read as bytes, or as pages — is off by 1024x or 4x and still looks like a memory reading. Too high kills healthy runs (how US-006 surfaced it), too low and the watchdog stops guarding without saying so; the RSS fallback makes the total neither metric | `proc-memory.test.js` |

Every row above was written assertion-first — the guard reviewed before the
implementation — except these six, which are covered test-*alongside* and
escalate on their next change: **Scheduler claim**, **Slot math**,
**Redaction**, **Magic-link consume**, **Tenant isolation**, **Slot collapse**
(offered as a candidate while it was being built, ruled in after it shipped).
...
outlives it (what pg-mem cannot prove, which layer lies about what) belongs in
`docs/testing.md`. Writing that account into this table instead is what made it
unreadable once already: it reached 5,479 words across 29 append-only commits
before anyone read it end to end, and for the last eight of those the table had
already stopped rendering as a table.
