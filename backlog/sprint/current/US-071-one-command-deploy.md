...
`scripts/deploy.sh <stack>` — where `<stack>` is `production`, `staging` or
`demo` — takes the argument that was previously spread across a project
...
1. **Print the plan and stop.** Stack, project name, env file (or the explicit
   ...
   to serve. Production asks for a confirmation; the other two do not.
2. **Set the environment itself.** `export ENV_FILE=…` for the two stacks that
   name one, `unset ENV_FILE` for production. Never inherit it. This is the
   whole of the 2026-08-05 fix.
3. **Deploy.** `pull` then `up -d`, with staging's `--policy always` — no stack
   builds what it serves.
...
- [ ] `scripts/deploy.sh <stack>` deploys any of the three app stacks with the
      right project name, env file and compose files, and is what all three
      runbooks tell the operator to run
...
- [ ] `DEPLOY.md` and the three app runbooks replace their copy-paste compose lines
