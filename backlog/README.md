...
|---|---|---|---|
| [BUG-008](sprint/current/done/BUG-008-unique-violation-matched-on-message.md) | `isUniqueViolation` ORed the exact `23505` with a `/unique\|duplicate/i` match on the message, so any error whose text said either word was answered as a name clash — asserting something false about the user's data | ✅ Fixed 2026-07-28 — the code alone, in one place both routes import. The pg-mem case the fallback was written for sets `23505` itself, so the guess behind the OR had only ever been able to catch what it was not written for. An engine's error *message* is not an interface | `server/src/routes/helpers.js`, `server/test/unique-violation.test.js` |
| [BUG-007](sprint/current/done/BUG-007-server-suite-fails-intermittently.md) | The server suite fails about one run in five, on a different test each time. CI does not run on a push to `dev`, so this is the gate it undermines | ✅ Fixed 2026-07-28 — two causes, neither the oversubscription first blamed: holding four cores busy changed nothing, while one file run *alone* failed 4 times in 40. pg-mem's bytea corruption is not always silent — 1.6% of ciphertexts build SQL its own parser rejects, and its error names a `kw_unique` token, so the paste route answered 409 for a name nothing had used. The other was a drain that spent 7.8s of an 8s budget every run; stub holds now free a named slot on a file, not a timer | `server/test/helpers/stored-key.js`, `server/test/stubs/fake_agent.js` |
...
| [BUG-006](sprint/current/done/BUG-006-empty-scheduled-target-reports-a-run.md) | A schedule whose target has no tests stamps `last_run_at` anyway, so a schedule that tests nothing reads as one that just passed | ✅ Fixed 2026-07-28 — the claim writes `next_run_at` alone and `last_run_at` waits for a run to actually start, which silences four other skips that were lying the same way; the list returns `target_tests` and an empty target says so on its row. `count(*)` is bigint and node-pg returns bigint as a *string* — a type-parser blind spot no pg-mem test can fail on | `server/src/scheduler.js`, `server/src/routes/schedules.js`, `frontend/src/SchedulesView.jsx` |
