...
| [US-038](sprint/current/done/US-038-staging-environment.md) | Staging environment (`staging.qassist.run`) | ✅ **Closed** 2026-07-26, 6/8 proven here — up and serving on its own LE cert, seeded, WS live view proven, US-008's snippet and the mail path closed against it, and four real defects found that no fixture could have (Traefik v3.3 vs Docker 29, `DEPLOY.md`'s own `ENV_FILE` prefix, the NULL `current_period_end` that became US-051, the Portal's scheduled-not-immediate cancel). The Stripe round trip ticked once US-051 shipped and re-proved on staging; the two criteria needing production, plus the round trip's two slivers (live endpoint isolation, 402-after-immediate-cancel), moved to US-056 | US-007 |
...
| [US-052](sprint/current/done/US-052-staging-branch-continuous-deploy.md) | Staging deploys from a branch, not a release | ✅ **Done** 2026-07-26 — **the loop is proven end to end, rollback included.** `staging` branched and pushed; the first build ran the full suite then published `:staging` and `:staging-15e7de3` at one digest in 3m5s, and `:latest` is provably untouched — still the same digest as `:0.2.3`, which is the criterion the tag split exists for. `main` was reconciled into `dev`, but **that first attempt did not take** — found 2026-07-26 while closing US-055: `15e7de3` merged a stale local `main` while `origin/main` had already moved to `32aa949`, so `git merge --ff-only staging` went on aborting. Fixed by `9f07713` (merge `origin/main` into `dev`, fast-forward `staging` onto it, no content change); the `--ff-only` promotion now succeeds. **2026-07-26, on the box:** `.env.staging` moved off `:0.2.3` onto `:staging` (backup at `.env.staging.bak-us053`) and US-053 deployed through it — `pull` then `up -d`, confirmed by digest and by the `org.opencontainers.image.revision` label reading `1c16eb9`, which is the check the mutable tag exists to need. **The rollback drill ran 2026-07-26** and was the last box-side unknown: with the box at tip (`3f3c6b0`), pinning `:staging-9f07713` and running the same `pull` + `up -d` recreated the container onto exactly that commit — revision label `9f07713…`, healthy, clean boot against the newer schema, since the migration runner skips applied rows it has no file for — and restoring `:staging` returned it to tip with `.env.staging` byte-identical to its pre-drill backup. Closed with the first live promotion **deferred to the first release**: it is a release decision, not an engineering unknown — the `--ff-only` is verified, and `release.yml` fires on the tag ref regardless of branch, proven end to end by `v0.2.3`. The first release exercises it in passing | US-032, US-038 |
...
| [US-054](sprint/current/done/US-054-activation-window-after-subscribe.md) | The activation window: capacity before the first run | ✅ **Done** 2026-07-26 — `ACTIVATION_SLA_HOURS` (unset or `0` = off) makes a paid account wait in a stated window while the operator adds the capacity it bought: a fourth step on US-053's checklist with the deadline in the reader's own words, 503 + `Retry-After` + `activation_pending` at all seven run-start paths, sticky operator-owned `users.activated_at`, `npm run activate` on the box, and two mails. Correctness-critical, assertion-first: `activation-gate.test.js` was written and reviewed before `activation.js` existed. **Three decisions the story did not make.** The two gates are ONE middleware and one DB read, so a route cannot have the billing check and miss this one. `010` **backfills** `activated_at` for every account that already has a subscription — without it, switching the feature on walls every existing customer at once, which is the story's own third failure mode arriving by deployment. And turning it back off *releases* everyone mid-window (off resolves to "activated", not "skip the check"), so an instance that outgrows rationing drops the line and restarts. **The live round trip closed the same day, on preview** at `fb4bcfa`: a real account subscribed through test-mode Checkout, sat walled on the fourth step, `npm run activate` over SSH listed it (20h 46m left) then granted it, the wall fell in the open tab, both mails arrived for real, and the next run started — carried by preview only because the same commit gave it Stripe test mode and real mail, the first payoff of reversing US-055's rule | US-022, US-053 |
| [US-055](sprint/current/done/US-055-preview-environment.md) | A preview environment, off to the side of the chain | ✅ **Done** 2026-07-26 — **live**, `preview.qassist.run` on its own LE cert, `noindex`, its own `pgdata`, built on the box from a `qassist:preview` tag no registry has held. Three loops end to end: a `wip/` branch never on `dev` was previewed, and `up -d` recreated from a new image ID with no tag change and no registry, confirmed by the revision label the build stamps (`dceb6c6` → `d4d4e42` → `41b3516`) — a registry-less image has no other way to say which commit it is. A preview cookie forced onto staging and demo with an explicit `Cookie:` header is 401 on both; a sign-in link for a stranger was printed to the log, not sent. `ci.yml` no longer runs on pushes to `dev`. **The `billing:false` / console-mail half of the design was reversed 2026-07-26** while proving US-054: it excluded every billing change from the fast loop and saved none of the CI-and-registry bill preview exists to skip, so preview now runs Stripe test mode and real mail on its own secrets. The story carries the superseding note; the boundary that remains is a populated database, a CI-built image and the promotion chain. **Three of the story's own claims were wrong and are corrected in it:** ~2 min is really 2–4 s; `docker image prune` reclaims *nothing* (BuildKit leaves no dangling image — the **build cache** grows, so it is `buildx prune --max-used-space`); and the box always had a working tree, so the new exception is *building*, which is also why preview needs a clone of its own. **The ninth criterion was failing on arrival and not because of preview:** `merge --ff-only staging` into `main` aborted, inherited from US-052's reconciliation having merged a stale local `main`. Fixed in `9f07713` — `origin/main` merged into `dev` for real, `staging` fast-forwarded onto it, no content change, and the promotion verified in a detached worktree rather than by moving `main` | US-038, US-052 |
| [US-008](sprint/current/done/US-008-cicd-integration.md) | CI/CD trigger: the documented pipeline step | ✅ **Done** 2026-07-26 — `docs/ci.md`'s script, extracted **verbatim** (sha256 `ee951934…`), run by **real GitHub Actions** against staging (run 30206357362): module by slug green, suite by uuid green, mixed suite **red** on a genuine `failed` verdict — the gate failing a build, not just reporting. All four runs in staging's `runs` table with `trigger = 'ci'` and the overridden `start_url`; branch, workflow and staging key deleted after, since the deliverable is the doc. **The runner found what the shell could not:** the permalink printed `***/runs/<id>` because the doc stored `QASSIST_URL` as a *secret* and Actions redacts secret values from logs, so every link it promised was unopenable — the base URL is a public hostname and now goes in GitHub `vars`, unmasked on GitLab, token only as a secret. GitLab stays **documented, not executed** by decision: the script is shell-identical, and its one GitLab-specific risk was the masking the Actions run already caught. Noted for whoever next touches `routes/tests.js`: `POST /api/tests` silently drops `project`/`module` slug keys, filing the test ungrouped with a 201 | US-007, US-009 |
...
| [US-032](sprint/current/done/US-032-release-pipeline-and-image.md) | CI on every push, a published image on every tag | ✅ **Done** 2026-07-26 — `v0.1.0` published to ghcr, anonymously pullable; the last criterion (the published image on a machine that never saw the source) closed in a clean directory rather than on the box: the compose file and `.env.example` curled from `main` per the quick start, image pulled anonymously, migrations applied at boot, UI 200 on :8080. Note: `main`'s quick start hands out `0.2.1` until the first `--ff-only` promotion lands (deferred in US-052) | US-031 |
...
That accepted risk came due, and cheaply: US-008 closed 2026-07-26 by editing
`docs/ci.md` on a public repo, because running the snippet from a real runner
showed Actions redacting the `QASSIST_URL` the doc had told readers to keep
secret. A line gained after publication, exactly as budgeted for.
...
Added to the sprint 2026-07-26: **US-056** (production deployment), which is a
consolidation rather than new scope. US-007 and US-038 had both shipped and
proven everything short of a running production — and production was
deliberately not stood up — so each was a finished story held open by the same
missing stack. US-056 collects those criteria (five from US-007, two-and-two-
slivers from US-038) into the one ticket that *is* the stand-up, and both
stories closed into `done/` on what they proved. It also inherits the sprint's
two dangling threads that only production resolves: US-040's demo CTA landing
on the proxy's default cert, and US-052's first `--ff-only` promotion, deferred
to the first release — which the stand-up is.
...
8. **US-007 → US-038 → US-008** — public HTTPS, then staging, then the
   ...
   **US-007 and US-038's repo halves shipped together on 2026-07-25**, in that
   ...
   no branch anywhere. Both stories stayed open until 2026-07-26, when their
   remaining production-only criteria were consolidated into US-056 (the
   production stand-up) and both closed on what they had proven.
...
`--ff-only` promotion US-052 exists to guarantee.

The keeping-honest constraint was written into `.env.preview.example` as "no
Stripe, no real mail" — and **that half was wrong, corrected 2026-07-26 while
proving US-054**. The bill preview skips is CI, an image push and a pull; env
vars are not part of it, so the rule saved nothing and put every billing and
entitlement change permanently on the slow loop. Preview now runs both, in
Stripe test mode and on its own Resend key and webhook secret. What actually
separates it from staging — and what those paragraphs were really protecting —
is a *populated* database, a CI-built image from a reviewed commit, and the
promotion chain. Those are the constraint now.
