...
| [US-055](sprint/current/US-055-preview-environment.md) | A preview environment, off to the side of the chain | 🟡 2/9 (2026-07-26) — **the repo half is in.** `.env.preview.example` (looser on purpose, and it says why: console mail, no Stripe, one session), `DEPLOY.md`'s Preview section with its three costs — a working tree on the box, disk, RAM — and `CLAUDE.md` / `CONTRIBUTING.md` / `README.md` all calling preview a spur rather than a stage. `ci.yml` no longer runs on pushes to `dev`; a PR into `dev` and a push to `staging` still run the full suite. Two criteria close on the repo alone: no `docker-compose.preview.yml`, and no compose file mentions preview. The documented build stamps `org.opencontainers.image.revision`, because a registry-less image has no other way to say which commit it is and the criterion asks for the running commit, not the tag. **The other seven need the box** | US-038, US-052 |
