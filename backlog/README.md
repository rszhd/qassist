...
|---|---|---|---|
| [BUG-007](bugs/BUG-007-server-suite-fails-intermittently.md) | The server suite fails about one run in five, on a different test each time — the runner oversubscribes the box and wall-clock stub holds miss their deadlines. CI does not run on a push to `dev`, so this is the gate it undermines | 🐛 Open (2026-07-28) | `server/package.json`, `server/test/stubs/fake_agent.js`, the `pollUntil` tests |
