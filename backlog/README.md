...
| [US-039](sprint/current/US-039-byok-only-no-server-key.md) | BYOK only: remove the server `OPENAI_API_KEY` | 📝 Filed 2026-07-25 — staging is publicly registrable and `requireEntitled` gates nothing when `STRIPE_*` is empty, so a stranger could spend the operator's key (US-038 wrote this down rather than fixing it). Assertions drafted, awaiting review — `resolveRunKey` is correctness-critical | US-005, US-021 |
